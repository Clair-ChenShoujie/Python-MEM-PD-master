...
# 定义目标函数
Z1 = lpSum([x[(i, j)] * data.loc[(data['目标样'] == '目标样{}'.format(i)) & (data['标准样'] == '标准样{}'.format(j)), '色差'].values[0] for i in range(1, 11) for j in range(1, 729)]) / average_delta
...
prob += lpSum([y[j] for j in range(1, 729)]) == 10
