...
#     cen = 4
#     if base_ks is None:
#         red_des = round(np.random.uniform(0, 0.05)*des, cen)
#         yel_des = round(np.random.uniform(0, 0.03)*des, cen)
#         blu_des = round(np.random.uniform(0, 0.03)*des, cen)
#     else:
#         red_des, yel_des, blu_des = base_ks
#         red_des += round(np.random.uniform(-0.01, 0.01)*des, cen)
#         yel_des += round(np.random.uniform(-0.01, 0.01)*des, cen)
#         blu_des += round(np.random.uniform(-0.01, 0.01)*des, cen)
        
#         # 确保数据在有效范围内
#         red_des = round(bound_value(red_des,0,0.05),cen)
#         yel_des = round(bound_value(yel_des,0,0.05),cen)
#         blu_des = round(bound_value(blu_des,0,0.05),cen)
...
# def aim_LAB(R):
#     df_R = pd.read_excel("2023test2/附件3.xlsx")
#     np_R = np.array(df_R)[]
...
def simulated_annealing(i, initial_temp=1000, final_temp=1, cooling_rate=0.95, max_iter_per_temp=100):
...
#     cost = 2*(60*red_des + 65*yel_des + 63*blu_des)
    
#     aver_E = 22
#     aver_C = 8.3
#     result = 0.5*delta_E/aver_E + 0.5*cost/aver_C
#     return red_des ,yel_des, blu_des,result

# Test the function
print(simulated_annealing(10))
